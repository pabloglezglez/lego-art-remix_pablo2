...
// Variable global para mantener el stud map y su orden
let globalAvailableStudHexList = null;
try {
...
function addWaterMark(pdf, isHighQuality) {
    for (let i = 0; i < pdf.internal.getNumberOfPages(); i++) {
        ...
        // Mover watermark y versión a la parte inferior
        const margin = 20;
        const x = pdf.internal.pageSize.width / 2;
        const y1 = pdf.internal.pageSize.height - margin;
        const y2 = pdf.internal.pageSize.height - margin + 10;
        pdf.text("Generated by lego-art-remix.com", x, y1, { align: "center" });
        pdf.text(VERSION_NUMBER, x, y2, { align: "center" });
    }
}
...
async function generateInstructions() {
    ...
    runStep4(async () => {
        ...
        // Guardar el stud map global para numeración consistente
        globalAvailableStudHexList = filteredAvailableStudHexList;
        generateInstructionTitlePage(
            resultImage,
            targetResolution[0],
            PLATE_WIDTH,
            globalAvailableStudHexList,
            SCALING_FACTOR,
            step4CanvasUpscaled,
            titlePageCanvas,
            selectedPixelPartNumber
